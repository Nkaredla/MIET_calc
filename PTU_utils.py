...
def tttr2bin(
    ...
    *,
    ...
    Python implementation of TTTR binning:
    ...
    binwidth : float
        Width of a macro bin in the same units as y (uses floor(y/binwidth)).
    flag : None, ndarray(bool), or ndarray(int)
        - None: bin all photons
        - bool mask: select photons (logical flag branch)
        - int labels: produce one column per label in min(flag)..max(flag)
    rebase : bool
        If True (recommended for streaming/chunked processing):
            subtract the minimum binned time so bins start at 0 (effectively
    ...
    - The original code has a padding trick in the flag branches to avoid edge issues
    ...
    # --- helper: core 1D binning, equivalent semantics ---
    def _bin_1d(y_sub: np.ndarray) -> np.ndarray:
        if y_sub.size == 0:
            return np.zeros((0,), dtype=np.int64)

        # Binning: y = floor(y/binwidth)
        yb = np.floor(y_sub.astype(np.float64) / float(binwidth)).astype(np.int64)

        # Original: y = y - min(y) + 1  (shift to start at 1)
    ...
    if isinstance(flag, (bool, np.bool_)) or (
        ...
        # Original implementation does a padding trick and then subtracts 1 from first and last bins.
    ...
    # Uses contiguous labels: flagv = min(flag):max(flag)
    lo = int(np.min(labels))
    hi = int(np.max(labels))
    flagv = np.arange(lo, hi + 1, dtype=labels.dtype)

    # To match expected output shape, we need a common nbins across channels.
...
def tttr2bintcspc(
    ...
    This function is a streaming-safe Python implementation of the call
    ...
    - Semantically equivalent to the original implementation when applied
